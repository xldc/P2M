...
import com.p2m.core.internal.event.mutable.InternalLikeLivaDataBackgroundLiveEvent
 ...
 */
interface P2MLikeLiveDataBackgroundLiveEvent<T> :
    P2MMutableBackgroundLiveEvent<T> {

    class Delegate<T> {
        private val real by lazy(LazyThreadSafetyMode.PUBLICATION) {
            InternalLikeLivaDataBackgroundLiveEvent<T>()
        }

        operator fun getValue(
            thisRef: Any?,
            property: KProperty<*>
        ): P2MLikeLiveDataBackgroundLiveEvent<T> = real

        operator fun setValue(
            thisRef: Any?,
            property: KProperty<*>,
            value: P2MLikeLiveDataBackgroundLiveEvent<T>
        ) = Unit
    }
